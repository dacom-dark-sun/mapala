...
class CommentModel extends ActiveRecord
{
     ...
     */
    public static function getTree($entity, $relatedTo, $maxLevel = null, $showDeletedComments = true)
    {
        ...
        if (!empty($models)) {
            $models = self::buildTree($models, $root_article);
        }
     ...
     */
    protected static function buildTree(&$data, $rootID)
    {
        $tree = [];

        foreach ($data as $id => $node) {
            $parent_permlink = $node->parent_permlink;
            if ($parent_permlink === $rootID) {
                unset($data[$id]);
                $node->children = self::buildTree($data, $node->permlink);
                $tree[] = $node;
            }
        }
    ...
    public function deleteComment()
    {
        ...
        return $this->save(false, ['status', 'updatedBy', 'updated_at']);
    }
 ...
 //       }
  ...
  //          return $this->author->getAvatar();
    ...
    {
        return ArrayHelper::map(self::find()->all(), 'author', 'author.username'); //return ArrayHelper::map(self::find()->joinWith('author')->all(), 'createdBy', 'author.username');
    }
    ...
    {
        $query = self::find()->where(['entity' => $this->entity, 'permlink' => $this->permlink]);
